...
- **云端账号登录**：支持邮箱注册 / 登录云端账号，登录后可使用云端配音配额（云端 TTS 通道），并在官网管理设备与订阅；各端登录态、配额、音色列表云端同步。
...
除了自填 Azure Key，插件也支持登录云端账号，使用云端配音配额（云端 TTS 通道）：

- 在 [官网](https://momovoicesub.sxrec.com) 注册账号（邮箱 + 密码），或在插件内"设置 > 登录账号"用邮箱登录。
- 登录后，配音通道自动切换为云端通道，无需自行配置 Azure Key。
- 可在官网管理订阅套餐、设备绑定与配额；各端登录态、配额、音色列表云端同步。
- 设备有绑定上限，一台机器登录过多账号会被拦截。
- 忘记密码可在插件登录弹窗或官网登录页跳转找回。

> 自填 Azure Key 与云端账号两种通道在"设置 > 连接设置"中用选项卡切换：有自填 Key 时优先走本地 Key 路线，禁用自填 Key 或仅登录云端账号时走云端通道。
